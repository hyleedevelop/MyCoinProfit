...
extension CalcViewController {
    ...
    @objc internal func datePickerAction(_ sender: UIDatePicker) {
        ...
        if sender == self.calcView.buyStartDatePicker {
            ...
            self.calcView.buyStartDateTextField.text = selectedDateFormatter.string(from: selectedDate)
            self.buyStartDateStringToCalculate = convertedDateFormatter.string(from: selectedDate)
        }
        
        if sender == self.calcView.buyEndDatePicker {
            self.calcView.buyEndDateTextField.textColor = .label
            self.calcView.buyEndDateTextField.text = selectedDateFormatter.string(from: selectedDate)
            self.buyEndDateStringToCalculate = convertedDateFormatter.string(from: selectedDate)
        }
        
        if sender == self.calcView.sellDatePicker {
            self.calcView.sellDateTextField.textColor = .label
            self.calcView.sellDateTextField.text = selectedDateFormatter.string(from: selectedDate)
